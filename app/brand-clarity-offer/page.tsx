...
const processSteps = [
  {
    ...
    body: "A detailed set of questions about your business, your clients, and what you're working toward. This ensures we walk into Session One ready to go deep from the first minute.",
  },
  {
    number: "2",
    eyebrow: "SESSION ONE + TWO",
    title: "We build your strategy together",
    body: "Two 2-hour virtual sessions, spaced a few days apart. Session One covers audience and competitive landscape. Session Two focuses on differentiators and positioning. You're in the room as the strategy takes shape.",
  },
  {
    number: "3",
    eyebrow: "UNDER A MONTH",
    title: "Your Brand Playbook is delivered",
    body: "A clean 3-4 page document with everything we built, plus a one-page 'What Now?' section with channel direction and 2-3 messaging angles based on your specific audience and positioning.",
  },
] as const;
...
const faqs = [
  {
    ...
    answer: "No. This session is built for service-based businesses of any kind -- creative agencies, consultants, coaches, copywriters, marketers, and others. The methodology applies across industries because positioning challenges are fundamentally the same.",
  },
  {
    question: "I already have a brand. Is this still worth it?",
    answer: "Often yes -- and sometimes even more so. If you have an existing brand that isn't quite working, these sessions help you understand exactly why and what to do about it. You don't have to start over to benefit from a clearer strategy.",
  },
  {
    question: "How is this different from hiring a brand designer?",
    answer: "A designer works on how your brand looks. This works on what your brand says, who it speaks to, and how it positions you in your market. Most designers need this work done before they start, not after. This session gives you the strategy that makes design decisions easy.",
  },
  {
    question: "What if I want more comprehensive brand strategy work?",
    answer: "There's a larger engagement available that covers brand substance, messaging framework, and brand voice in full depth. This mini session is the entry point -- and the investment applies toward the larger engagement if you decide to continue.",
  },
  {
    question: "How quickly can we schedule?",
    answer: "Availability varies. Booking at least two to three weeks out is recommended. Current availability is shown on the booking page.",
  },
  {
    question: "What do I need to prepare?",
    answer: "You complete the intake questionnaire before Session One -- that's the main preparation. It takes about 30 to 45 minutes and ensures we make the most of our time together. No additional research or homework is required before your first session.",
  },
] as const;
...
const bookingTarget = BOOKING_URL.startsWith("http") ? { target: "_blank", rel: "noreferrer" } : {};
...
function HeroSection() {
  return (
    <section id="top" className="relative overflow-hidden bg-[var(--ss-ink)] pt-[64px] text-[var(--ss-light)] md:pt-[68px]">
      <div className="absolute inset-0">
        <Image src={HERO_IMAGE_SRC} alt="Founder in an office at dusk" fill priority sizes="100vw" className="object-cover object-[center_15%]" />
      </div>
          ...
          <div className="flex w-full justify-center">
            <div className="w-full max-w-[1200px] text-center">
              ...
              <FadeIn delay={0.48} className="mt-8 flex flex-wrap justify-center gap-4">
                <CtaLink href="#process" variant="primary" className="min-w-[176px] px-6 py-4">SEE HOW IT WORKS</CtaLink>
                <CtaLink href={BOOKING_URL} variant="secondary" className="min-w-[176px] px-6 py-4">Let&apos;s talk</CtaLink>
              </FadeIn>
            </div>
          </div>
        </div>
        ...
        <div className="divide-y divide-white/10 border-y border-white/10">
          {benefits.map((item, index) => (
            <FadeIn key={item.number} delay={0.08 * index} className="grid gap-2 py-3 sm:grid-cols-[72px_1fr] sm:gap-1 sm:py-3">
              ...
              <div>
                <h3 className="text-[22px] leading-[1.15] text-[var(--ss-light)] sm:text-[18px]">{item.title}</h3>
                <p className="mt-2.5 max-w-[540px] text-[14px] leading-6 text-[var(--ss-muted)]">{item.body}</p>
              </div>
            </FadeIn>
          ))}
        </div>
      ...
      <div className="mx-auto grid max-w-[1400px] gap-8 px-4 py-12 sm:px-6 md:py-14 lg:grid-cols-[0.9fr_1.1fr] lg:px-8 lg:py-16">
        <FadeIn>
          ...
          <p className="mt-6 max-w-[350px] text-[14px] leading-7 text-[var(--ss-dark-text)]/72">
            The whole point of this session is that it works and feels focused. Everything is designed to give you the highest-impact strategy work in the least amount of time — without cutting corners on depth.
          </p>
        </FadeIn>
        <div className="space-y-6">
          {processSteps.map((step, index) => (
            <FadeIn key={step.number} delay={index * 0.06} className="grid gap-4 border-t border-[var(--ss-soft-border)] pt-5 sm:grid-cols-[88px_1fr]">
              <div className="flex items-start gap-3 sm:block">
                <p className="font-[family:var(--font-cormorant)] text-[32px] leading-none text-[var(--ss-bronze)]/72">{step.number}</p>
                <p className="font-[family:var(--font-cinzel)] text-[10px] uppercase tracking-[0.2em] text-[var(--ss-bronze)]/68 sm:mt-3">{step.eyebrow}</p>
              </div>
              <div>
      ...
      <div className="mx-auto max-w-[1180px] px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
        <div className="grid gap-10 lg:grid-cols-[1fr_auto] lg:items-start">
          <FadeIn className="max-w-[760px]">
            <p className="font-[family:var(--font-cormorant)] text-[24px] italic text-[var(--ss-bronze-soft)]/78">What clients say</p>
            ...
            <div className="mt-8 border-t border-[var(--ss-bronze)]/25 pt-5">
              <p className="font-[family:var(--font-cinzel)] text-[10px] uppercase tracking-[0.24em] text-[var(--ss-bronze-soft)]">{active.author}</p>
              <p className="mt-2 text-[12px] uppercase tracking-[0.18em] text-[var(--ss-light)]/58">{active.role}</p>
            </div>
          </FadeIn>
          <FadeIn delay={0.08} className="flex items-center gap-3 lg:pt-8">
            <button type="button" onClick={() => setIndex((v) => (v - 1 + total) % total)} className="inline-flex size-11 items-center justify-center rounded-full border border-[var(--ss-bronze)]/25 text-[var(--ss-light)]/72 transition hover:border-[var(--ss-bronze-soft)] hover:text-[var(--ss-bronze-soft)]">
              <ArrowLeft className="size-4" />
            </button>
            <p className="text-[11px] uppercase tracking-[0.22em] text-[var(--ss-light)]/56">{String(index + 1).padStart(2, "0")} / {String(total).padStart(2, "0")}</p>
            <button type="button" onClick={() => setIndex((v) => (v + 1) % total)} className="inline-flex size-11 items-center justify-center rounded-full border border-[var(--ss-bronze)]/25 text-[var(--ss-light)]/72 transition hover:border-[var(--ss-bronze-soft)] hover:text-[var(--ss-bronze-soft)]">
    ...
    <section id="offer" className="bg-[var(--ss-cream)] px-4 py-14 text-[var(--ss-dark-text)] sm:px-6 md:py-16 lg:px-8 lg:py-[72px]">
      <div className="mx-auto max-w-[1180px]">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(310px,370px)] lg:items-start lg:gap-12">
          <div>
            {deliverables.map((d) => (
              <div key={d.title} style={{ display: "flex", gap: 16, alignItems: "flex-start", padding: "12px 0", borderBottom: "0.5px solid rgba(61,46,40,0.1)", fontSize: 15, color: T.ash, lineHeight: 1.45 }}>
      ...
      <div className="mx-auto grid max-w-[1180px] gap-10 px-4 py-14 sm:px-6 lg:grid-cols-[1fr_1fr] lg:px-2 lg:py-20">
        <FadeIn>
          ...
          <div className="mt-6 max-w-[430px] space-y-5 text-[14px] leading-7 text-[var(--ss-muted)]">
            <p>At the end of our journey, you&apos;ll receive a Brand Playbook. This document captures who you serve, where you stand in your market, what makes you different, and where you want to focus your energy.</p>
            <p>This playbook is your foundation, there to guide all the other decisions you make about your brand with renewed ease.</p>
          </div>
        </FadeIn>
        <FadeIn delay={0.08}>
      ...
      <div className="mx-auto max-w-[1400px] px-4 py-12 sm:px-6 md:py-14 lg:px-8 lg:py-16">
        <FadeIn>
          ...
          <p className="mt-5 text-[14px] leading-7 text-[var(--ss-dark-text)]/68">
            If something isn&apos;t covered here, you&apos;re always welcome to reach out before booking. A discovery call is available if you want to talk through whether this is the right fit.
          </p>
        </FadeIn>
          ...
          {faqColumns.map((column, columnIndex) => (
            <div key={columnIndex} className="space-y-3">
              {column.map((faq, itemIndex) => (
                <AccordionItem key={faq.question} question={faq.question} answer={faq.answer} delay={0.05 * (columnIndex * 3 + itemIndex)} />
              ))}
            </div>
          ))}
        </div>
    ...
    <section className="bg-[var(--ss-bronze)] text-[var(--ss-light)]">
      <div className="mx-auto flex max-w-[1400px] flex-col items-center px-4 py-12 text-center sm:px-6 md:py-14 lg:px-8 lg:py-16">
        <FadeIn>
          ...
          <CtaLink href={BOOKING_URL} target="_blank" rel="noopener noreferrer" variant="ghost" className="mt-7 px-7 py-4">
            Book now
          </CtaLink>
    ...
    <footer className="bg-[var(--ss-ink)] text-[var(--ss-light)]">
      <div className="mx-auto grid max-w-[1400px] gap-7 px-4 py-10 sm:px-6 md:py-12 lg:grid-cols-[1.2fr_0.7fr_0.7fr_1fr] lg:px-8">
        ...
        <div>
          ...
          <p className="mt-5 max-w-[260px] text-[15px] leading-7 text-[var(--ss-muted)]">Have questions or ready to elevate your brand? We&apos;d love to hear from you.</p>
...
export default function BrandClarityConviction() {
  return (
